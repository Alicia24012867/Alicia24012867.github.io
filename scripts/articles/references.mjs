...
export function createReferenceResolver(filename, resolveAsset, onArticleLink) {
  return (value, image = false) => {
    ...
    let decoded;
    try { decoded = decodeURIComponent(match[1]).replaceAll('\\', '/'); } catch { throw new Error(`${filename}: 无效的链接 ${value}`); }
    const localFile = path.posix.normalize(path.posix.join(path.posix.dirname(filename), decoded));
    if (localFile === '..' || localFile.startsWith('../') || path.posix.isAbsolute(localFile)) throw new Error(`${filename}: 本地链接不可越过 articles 目录：${value}`);
