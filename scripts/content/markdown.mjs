...
export function renderMarkdown(body) {
  ...
  const items = notes.map(({ reference, content }) => {
    const backlinks = Array.from(
      ...
      (_, index) =>
        `<a class="footnote-back" href="#fnref-${reference.anchor}-${index + 1}" aria-label="Back to reference ${index + 1} of footnote ${reference.number}">↩${reference.count > 1 ? index + 1 : ''}</a>`,
    ).join(' ');
    return `<li id="fn-${reference.anchor}"><div class="footnote-content">${content}<p>${backlinks}</p></div></li>`;
  });
  const footnotes = items.length
    ? `<section class="article-footnotes" aria-label="Footnotes"><p>Footnotes</p><ol>${items.join('')}</ol></section>`
    : '';
...
export function restoreMath(html, mathHtml) {
  ...
  return html.replace(
    /<(span|div) data-math-index="(\d+)">[^<]*<\/\1>/g,
    (match, _tag, index) => mathHtml[Number(index)] || match,
  );
}
